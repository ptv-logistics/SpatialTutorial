    ...
    public class TilesAndLabelsHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (var graphics = Graphics.FromImage(bmp))
            {
                ...
                int bleedingPixels = Math.Max(strokeSize, sz / 2);
                ...
                for (double lon = left; lon <= right; lon++)
                {
                    for (double lat = top; lat <= bottom; lat++)                    
                    {
                        ...
                        if (z > 6)
                        { 
                            graphics.FillEllipse(Brushes.LightGray, (int)(p1.X + p2.X) / 2 - sz, (int)p1.Y - sz, sz * 2, sz * 2);
                            graphics.FillEllipse(Brushes.LightGray, (int)(p1.X) - sz, (int)(p1.Y + p2.Y) / 2 - sz, sz * 2, sz * 2);
